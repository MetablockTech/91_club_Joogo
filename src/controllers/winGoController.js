...
import { REWARD_STATUS_TYPES_MAP, REWARD_TYPES_MAP } from "../constants/reward_types.js";
...
const distributeCommission = async (customStart = null, customEnd = null) => {
  try {
    const { startOfYesterdayTimestamp, endOfYesterdayTimestamp } = yesterdayTime();
    
    // We will check for the last 7 days by default to catch up on any missed payouts
    // If customStart is provided, we only process that specific range (for manual test)
    let dateRanges = [];
    if (customStart && customEnd) {
      dateRanges.push({ start: customStart, end: customEnd });
    } else {
      for (let i = 1; i <= 7; i++) {
        const d = new Date();
    ...
    console.log(`--- Starting Commission Distribution for ${dateRanges.length} periods ---`);

    for (const range of dateRanges) {
      const payoutDate = new Date(range.start).toISOString().split('T')[0];
      console.log(`Processing payout for date: ${payoutDate}`);

      // 1. Update user ranks for this period
      await rankController.updateUserRanks(range.start, range.end);

      // 2. Aggregate turnover (including API games)
      const [allBets] = await connection.query(
        `
        SELECT phone, game_type, SUM(money) AS total_money
        FROM (
          SELECT phone, 'wingo' AS game_type, (money + fee) AS money FROM minutes_1 WHERE time > ? AND time <= ?
          UNION ALL
          SELECT phone, 'k3' AS game_type, money FROM result_k3 WHERE time > ? AND time <= ?
          UNION ALL
          SELECT phone, '5d' AS game_type, money FROM result_5d WHERE time > ? AND time <= ?
          UNION ALL
          SELECT phone, 'trx_wingo' AS game_type, (money + fee) AS money FROM trx_wingo_bets WHERE time > ? AND time <= ?
          UNION ALL
          SELECT phone, 'jilli' AS game_type, betAmount AS money FROM jilliebethistory WHERE today BETWEEN FROM_UNIXTIME(?/1000) AND FROM_UNIXTIME(?/1000)
          UNION ALL
          SELECT phone, 'spribe' AS game_type, deposit_amount AS money FROM spribetransaction WHERE today BETWEEN FROM_UNIXTIME(?/1000) AND FROM_UNIXTIME(?/1000)
          UNION ALL
          SELECT u.phone, 'wc' AS game_type, wc.amount AS money FROM wc_api_transactions wc JOIN users u ON wc.user_id = u.id_user WHERE wc.type = 'debit' AND wc.is_cancel = 0 AND wc.created_at BETWEEN FROM_UNIXTIME(?/1000) AND FROM_UNIXTIME(?/1000)
        ) AS combined
        GROUP BY phone, game_type
        `,
        [
          range.start, range.end, range.start, range.end, range.start, range.end, range.start, range.end,
          range.start, range.end, range.start, range.end, range.start, range.end
        ]
      );

      // 3. Distribute commission with duplicate check
      for (const bet of allBets) {
        // Check if already paid for this user, game, and date
        const [existing] = await connection.query(
          "SELECT id FROM commission_payout_logs WHERE phone = ? AND game_type = ? AND payout_date = ?",
          [bet.phone, bet.game_type, payoutDate]
        );

        if (existing.length === 0) {
          // Process Commission
          await commissionController.rosesPlus(bet.phone, bet.total_money, bet.game_type, range.end);
          
          // Log the payout
          await connection.execute(
            "INSERT INTO commission_payout_logs (phone, game_type, payout_date, total_turnover) VALUES (?, ?, ?, ?)",
            [bet.phone, bet.game_type, payoutDate, bet.total_money]
          );
        }
      }
    }
    ...
    return {
      success: true,
      message: "Commissions processed and caught up successfully.",
    };
  } catch (error) {
...
const betWinGo = async (req, res) => {
  ...
  const [winGoNow] = await connection.query(
    "SELECT period FROM wingo WHERE status = 0 AND game = ? ORDER BY period DESC LIMIT 1",
    [gameJoin],
  );
  ...
  if (check >= 0) {
    ...
    const [illegalBets] = await connection.query(
      "SELECT COUNT(*) as totalBets FROM minutes_1 WHERE phone = ? AND status = 0 AND stage = ?",
      [userInfo.phone, period]
    );

    if (illegalBets[0].totalBets > 1) {
      const [bets] = await connection.query(
        "SELECT bet, game FROM minutes_1 WHERE phone = ? AND status = 0 AND stage = ? LIMIT 1",
        [userInfo.phone, period]
      );
      const betValue = bets[0]?.game ?? 0;
      console.log(betValue)
    }

    await connection.query(
      "UPDATE users SET money = money - ?, wingoGameTotalBetAmount = wingoGameTotalBetAmount + ?, totalBetAmount = totalBetAmount + ? WHERE token = ?",
      [totalBetMoney, totalBetMoney, totalBetMoney, auth],
    );
    ...
    await vipController.payoutRealTimeRebate(userInfo.phone, totalBetMoney);
...
const listOrderOld = async (req, res) => {
  ...
  const [wingo] = await connection.query(
    "SELECT * FROM wingo WHERE status != 0 AND game = ? ORDER BY period DESC LIMIT ?, ?",
...
const getWinningResultsForBet = (bet) => {
  const matches = [];
  ...
  for (const { name, bets } of betColumns) {
    if (bets.includes(bet)) {
      matches.push(...(colorBets[name] || []));
    }
  }
...
const calculatePayout = (bet, total, result) => {
  let payout = 0;
  ...
  if (bet === "l" || bet === "n") {
    ...
    if (result === 0 || result === 5) {
      ...
      if (bet === resultStr) {
        payout = total * 9;
      } else if ((result % 2 === 0 && bet === "d") || (result % 2 === 1 && bet === "x")) {
        payout = total * 2;
      }
    }
  }
...
const findSafeResult = (bet, total, deposit, userMoney, totalWithdrawal, totalRechargeCount, totalWithdrawalCount) => {
  console.log(bet, total, deposit, userMoney, totalWithdrawal, totalRechargeCount, totalWithdrawalCount);

  if (totalRechargeCount == totalWithdrawalCount) {
    return "false";
  } else if (totalWithdrawalCount > totalRechargeCount) {
    return "false";
  }

  const totalBase = Math.max(deposit); // use the bigger one
  const maxAllowedPayout = totalBase * 0.7;

  console.log("maxAllowedPayout +>", totalBase, maxAllowedPayout)

  const possibleResults = getWinningResultsForBet(bet); // might return [0,2,4,6,8]

  const safeResults = possibleResults.filter((res) => {
    const payout = calculatePayout(bet, total, res);

    // First check: payout should be less than maxAllowedPayout
    if (payout < maxAllowedPayout) {
      // Second check: payout + userMoney should also be less than maxAllowedPayout
      return (payout + userMoney) < maxAllowedPayout;
    }

    return false; // If payout itself exceeds maxAllowedPayout, it's not safe
  });

  console.log("safeResults", safeResults);

  if (safeResults.length > 0) {
    // Pick random safe result
    return safeResults[Math.floor(Math.random() * safeResults.length)];
    //  return Math.floor(Math.random() * 10);
  }

  // If no safe result, fallback to 0
  return "false";
};
...
//         const user = minPlayers[0];
//         const phone = user.phone; // or user.id_user if available
//         const userBet = user.bet;

//         // Get total recharge and withdrawal
//         const [rechargeRows] = await connection.query(
//           "SELECT money AS total_recharge FROM recharge WHERE phone = ? AND status = 1 ORDER BY id DESC LIMIT 1",
//           [phone]
//         );

//         //total recharge count
//         const [rechargeCount] = await connection.query(
//           "SELECT COUNT(*) AS total_recharge_count FROM recharge WHERE phone = ? AND status = 1",
//           [phone]
//         );

//         const [withdrawalRows] = await connection.query(
//           "SELECT money AS total_withdrawal FROM withdraw WHERE phone = ? ORDER BY id DESC LIMIT 1",
//           [phone]
//         );

//         //total withdrawal count
//         const [withdrawalCount] = await connection.query(
//           "SELECT COUNT(*) AS total_withdrawal_count FROM withdraw WHERE phone = ?",
//           [phone]
//         );

//         const [userMoney] = await connection.query(
//           "SELECT money FROM users WHERE phone = ?",
//           [phone]
//         );


//         const totalRecharge = parseFloat(rechargeRows[0]?.total_recharge || 0);
//         const totalWithdrawal = parseFloat(withdrawalRows[0]?.total_withdrawal || 0);
//         const totalUserMoney = parseFloat(userMoney[0]?.money || 0);

//         const total = parseFloat(user.money) || 0;

//         amount = findSafeResult(userBet, total, totalRecharge, totalUserMoney, totalWithdrawal, rechargeCount[0]?.total_recharge_count, withdrawalCount[0]?.total_withdrawal_count);

//         isUserLost = amount == "false";
...
//         let result = await getWingoResult(game)
//         let finalResult;
...
//           [finalResult, previousPeriod, join],
//         );
//         newArr = "-1";
//       } else {
//         // admin set result
...
//      // Get period from API
//     const period = await getWingoPeriod(game);

//     // Prepare the insert query
//     const query = `
//       INSERT INTO wingo (period, amount, game, status, time)
//       VALUES (?, 0, ?, 0, ?)
//     `;

//     // If no period from API, generate a new one
//     const finalPeriod = period || generatePeriod(GameRepresentationIds.WINGO[game]);

//     // Execute the query with prepared statement
//     await connection.query(query, [finalPeriod, join, timeNow]);
//   } catch (error) {
//     if (error) {
//       console.log(error);
//     }
...
const addWinGo = async (game) => {
  try {
    ...
    // Clean up any duplicate period rows (keep only the one with the lowest id)
    await connection.query(
      `DELETE w1 FROM wingo w1
       INNER JOIN wingo w2
       ON w1.period = w2.period AND w1.game = w2.game AND w1.id > w2.id
       WHERE w1.game = ?`,
      [join],
    );
    ...
    if (isPendingGame) {
      ...
      if (minPlayers.length >= 2) {
        ...
        const categories = await Promise.all(totalMoneyPromises);
        const minMoney = Math.min(...categories.map((c) => c.total_money));
        const minCategories = categories.filter((c) => c.total_money === minMoney);
        const smallestCategory =
          minCategories[Math.floor(Math.random() * minCategories.length)];
        const colorBets = {
          red_6: [6],
          red_8: [8],
          red_2: [2], //0 removed
    ...
    if (isPendingGame) {
      // Derive next period from the one we just resolved to avoid boundary timing issues
      const prevStr = winGoNow[0].period.toString();
      const prevDatePart = prevStr.substring(0, 8);
      const gameIdPart = prevStr.substring(8, 10);
      const prevIncrement = parseInt(prevStr.substring(10));
      const todayDate = moment().tz("Asia/Kolkata").format("YYYYMMDD");

      if (todayDate !== prevDatePart) {
        // Day rolled over, start from period 0 of the new day
        NewGamePeriod = parseInt(`${todayDate}${gameIdPart}0000`);
      } else {
        const nextIncrement = prevIncrement + 1;
        NewGamePeriod = parseInt(
          `${prevDatePart}${gameIdPart}${String(nextIncrement).padStart(4, "0")}`,
        );
      }
    } else {
      // No pending game (e.g., server just started) — compute from current time
      NewGamePeriod = generatePeriods(gameRepresentationId, gameDurationInSeconds);
    }

    // Only insert if this period doesn't already exist for this game
    const [existingPeriod] = await connection.query(
      "SELECT id FROM wingo WHERE period = ? AND game = ? LIMIT 1",
      [NewGamePeriod, join],
    );
...
const handlingWinGo1P = async (typeid) => {
  try {
    ...
    const [winGoNow] = await connection.query(
      "SELECT * FROM wingo WHERE status = 1 AND release_status = 1 AND game = ? ORDER BY period DESC LIMIT 1",
      [game],
    );
    ...
    switch (result) {
      case 0:
        await connection.query(updateStatusSQL, [game, "t", "n", "d", "0"]);
        break;
      case 1:
        await connection.query(updateStatusSQL, [game, "l", "n", "x", "1"]);
        break;
      ...
      case 4:
        ...
        break;
      case 5:
        await connection.query(updateStatusSQL, [game, "l", "t", "x", "5"]);
        break;
      case 6:
        await connection.query(updateStatusSQL, [game, "l", "n", "d", "6"]);
        break;
    ...
    for (let i = 0; i < order.length; i++) {
      ...
      let totals = parseFloat(nhan_duoc);
      await connection.query(
        "UPDATE `minutes_1` SET `get` = ?, `status` = 1 WHERE `id` = ?",
        [parseFloat(nhan_duoc), id],
      );
      const sql = "UPDATE `users` SET money = money + ? ,wingo_totalWinningAmount = wingo_totalWinningAmount + ?,totalWinningAmount =totalWinningAmount + ? WHERE `phone` = ?";
      await connection.query(sql, [totals, totals, totals, phone]);
    }
