...
export const spribeLaunchGame = async (req, res) => {
    ...
    try {
        ...
        const return_url = settings[0]?.website_link || "https://starworldz.com";
...
export const spribeDeposit = async (req, res) => {
    ...
    try {
        ...
        // Update the user's balance and total betting stats
        await connection.query('UPDATE users SET money = ?, totalBetAmount = totalBetAmount + ? WHERE id_user = ?', [new_balance, amount, user_id]);

        // 7-Level Commission Distribution
        await commissionController.rosesPlus(user.phone, amount, 'spribe_game');
        
        // Update Turnover Statistics
        await commissionController.updateTurnover(user.phone, amount, user.code, user.invite);
...
export const spribeWithdraw = async (req, res) => {
    ...
    try {
        ...
        // Update the user's balance and winning statistics
        await connection.query(
            'UPDATE users SET money = ?, totalWinningAmount = totalWinningAmount + ?, spribe_totalWinningAmount = spribe_totalWinningAmount + ? WHERE id_user = ?', 
...
export const spribeRollback = async (req, res) => {
    ...
    try {
        ...
        // Update the user's balance to reflect the rollback and reverse betting stats
        await connection.query('UPDATE users SET money = ?, totalBetAmount = totalBetAmount - ? WHERE id_user = ?', [new_balance, amount, user_id]);
