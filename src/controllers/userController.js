...
import { getTodayStartTime, generateClaimRewardID } from "../helpers/games.js";
import rankController from "./rankController.js";
import { getOTPExpiry, getOTPCooldown } from "../helpers/smsService.js";
...
const verifyCode = async (req, res) => {
  ...
  let timeEnd = now + (expiryMinutes * 60 * 1000);
  ...
  // Anti-spam check in otp_verify (Dynamic cooldown)
  const [otpRows] = await connection.query("SELECT time_otp FROM otp_verify WHERE phone = ?", [user.phone]);
...
const userInfo = async (req, res) => {
  ...
  let totalRecharge = rechargeResult.total || 0;

  const [[withdrawResult]] = await connection.query(
    "SELECT SUM(money) AS total FROM withdraw WHERE `phone` = ? AND status = 1",
    [rows[0].phone],
  );
  let totalWithdraw = withdrawResult.total || 0;

  const { id, password, ip, veri, ip_address, status, time, loginTime, token, ...others } =
    rows[0];
...
const promotion = async (req, res) => {
  ...
  const [user] = await connection.query(
    "SELECT `phone`, `code`,`invite`, `roses_f`, `roses_f1`, `roses_today`, `rebet_level` FROM users WHERE `token` = ? ",
    [auth],
  );
  console.log("Promotion API - User found:", user?.[0]?.phone, "Code:", user?.[0]?.code);
  ...
  const [f1s] = await connection.query(
    "SELECT u.`phone`, u.`code`,u.`invite`, u.`time`, IFNULL(t.daily_turn_over, 0) as daily_turn_over, IFNULL(t.total_deposit, 0) as total_deposit FROM users u LEFT JOIN turn_over t ON u.phone = t.phone WHERE u.`invite` = ? ",
    [userInfo.code],
  );
  ...
  // Recursive function to fetch subordinates from turn_over table
  async function fetchInvitesByCode(code, depth = 1) {
    if (depth > 7) return;

    const [subordinates] = await connection.query(
      "SELECT u.`phone`, u.`code`, u.`invite`, IFNULL(t.daily_turn_over, 0) as today_turnover, IFNULL(t.daily_deposit, 0) as today_deposit FROM users u LEFT JOIN turn_over t ON u.phone = t.phone WHERE u.`invite` = ?",
      [code]
    );

    if (subordinates.length > 0) {
      for (const sub of subordinates) {
        const turnover = parseFloat(sub.today_turnover) || 0;
        const deposit = parseFloat(sub.today_deposit) || 0;

        const subObj = {
          ...sub,
          today_turnover: turnover,
          today_deposit: deposit,
          depth: depth
        };

        selectedData.push(subObj);
        await fetchInvitesByCode(sub.code, depth + 1);
      }
    }
  }

  // Execute recursive fetch using the turn_over table
  await fetchInvitesByCode(userInfo.code, 1);

  // Totals based on turn_over table values
  const teamCount = selectedData.length;
  const today_turnover = selectedData.reduce((sum, item) => sum + item.today_turnover, 0);
  const today_deposits = selectedData.reduce((sum, item) => sum + item.today_deposit, 0);

  // Calculate Projected Rank (Dynamic Estimation)
  let projected_level = 0;
  const thresholds = await rankController.getAllRankThresholds();
  for (let i = thresholds.length - 1; i >= 0; i--) {
    const t = thresholds[i];
    if (teamCount >= t.min_team && today_turnover >= t.min_betting && today_deposits >= t.min_deposit) {
      projected_level = t.level;
      break;
    }
  }

  // Fetch rates for the projected level
  const [allLevelRates] = await connection.query("SELECT * FROM level");
  const myLevelData = allLevelRates.find(l => l.level === projected_level) || allLevelRates[0] || {};

  let estimated_income = 0;
  let level_stats = {};
  for (let i = 1; i <= 7; i++) {
    const rate = parseFloat(myLevelData[`f${i}`]) || 0;
    const levelTurnover = selectedData.filter(d => d.depth === i).reduce((sum, item) => sum + item.today_turnover, 0);
    const income = levelTurnover * (rate / 100);
    level_stats[i] = {
      turnover: levelTurnover,
      rate: rate,
      income: income
    };
  ...
  return res.status(200).json({
    ...
    status: true,
    invite: {
      f1: selectedData.filter(d => d.depth === 1).length,
      total_f: teamCount,
      f1_today: f1_today || 0,
      f_all_today: f_all_today || 0,
      roses_f1: Number(userInfo.roses_f1 || 0).toFixed(3),
      roses_f: Number(userInfo.roses_f || 0).toFixed(3),
      roses_all: Number(rosesAdd || 0).toFixed(3),
      roses_today: Number(userInfo.roses_today || 0).toFixed(3),
      today_turnover: Number(today_turnover || 0).toFixed(2),
      today_deposits: Number(today_deposits || 0).toFixed(2),
      estimated_income: Number(estimated_income || 0).toFixed(3),
...
const addBank = async (req, res) => {
  ...
  try {
    ...
    if (!auth || !bankName || !recipientName || !bankAccountNumber || !email || !tinh) {
      return res.status(200).json({
        message: "Failed",
        status: false,
        timeStamp: timeNow,
    ...
    const [user_bank] = await connection.query(
      "SELECT * FROM user_bank WHERE bank_phoneNo = ? ",
      [tinh],
    );
    ...
    if (user_bank.length == 0 && user_bank2.length == 0) {
      ...
      await connection.query("UPDATE user_bank SET bank_phoneNo = ? WHERE phone = ? ", [
        tinh,
        userInfo.phone,
      ]);
      return res.status(200).json({
      ...
      await connection.query(
        "UPDATE user_bank SET name_bank = ?, name_user = ?, account_number = ?, email = ?, address_alias = ?, bank_phoneNo = ?, time = ? WHERE phone = ?",
        [bankName, recipientName, bankAccountNumber, email, sdt, tinh, time, userInfo.phone],
      );
      return res.status(200).json({
...
const infoUserBank = async (req, res) => {
  ...
  try {
    ...
    const trxWingoLossMoney = Number(trxWingoLossResult[0].trxWingoLossMoney) || 0;

    // API Games Loss Calculations
    const [jilliLoss] = await connection.query(`SELECT SUM(betAmount) as bet FROM jilliebethistory WHERE phone = ? AND today > FROM_UNIXTIME(?/1000)`, [userInfo.phone, lastBetResetTime[0]?.time || 0]);
    const [spribeLoss] = await connection.query(`SELECT SUM(deposit_amount) as bet FROM spribetransaction WHERE phone = ? AND today > FROM_UNIXTIME(?/1000)`, [userInfo.phone, lastBetResetTime[0]?.time || 0]);
    const [wcLoss] = await connection.query(`SELECT SUM(amount) as bet FROM wc_api_transactions WHERE user_id = ? AND type = 'debit' AND is_cancel = 0 AND created_at > FROM_UNIXTIME(?/1000)`, [userInfo.id_user, lastBetResetTime[0]?.time || 0]);

    const apiLossMoney = parseFloat(jilliLoss[0].bet || 0) + parseFloat(spribeLoss[0].bet || 0) + parseFloat(wcLoss[0].bet || 0);
    ...
    const totalLoss =
      wingoLossMoney + k3LossMoney + G5dLossMoney + trxWingoLossMoney + apiLossMoney;
    ...
    if (calculation < 0 && lastRecharge.length > 0) {
      await connection.query(
        `UPDATE recharge SET remaining_bet = 0 WHERE id = ? AND status = 1`,
        [lastRecharge[0].id],
      );
    }
...
const withdrawal3 = async (req, res) => {
  ...
  let total2 = 0;
  // Use helper for comprehensive betting check in withdrawal3
  const stats = await userStatsHelper.getBettingStats(userInfo.phone);
  total2 = stats.totalAmount;
  let result = 0;
...
const transfer = async (req, res) => {
  ...
  let total2 = 0;
  // Use helper for comprehensive betting check in transfer
  const stats = await userStatsHelper.getBettingStats(userInfo.phone);
  total2 = stats.totalAmount;
...
const listRecharge = async (req, res) => {
  ...
  let sql = "SELECT * FROM recharge WHERE phone = ?";
  ...
  if (toDate) {
    sql += " AND time <= ?";
    // Add 23:59:59 to toDate to include the whole day
  ...
  const [recharge] = await connection.query(sql, params);
...
const useRedenvelope = async (req, res) => {
  try {
    ...
    let redenvelopes;
    try {
      [redenvelopes] = await connection.query(
        "SELECT * FROM redenvelopes WHERE id_redenvelope = ?",
        [code],
      );
    } catch (dbError) {
      console.error("Redenvelope Query Error:", dbError.message);
      return res.status(200).json({
        message: "Invalid code format. Please enter a valid gift code.",
        status: false,
        timeStamp: timeNow,
      });
    }
    ...
    await connection.query(
      "UPDATE users SET money = money + ?, red_envelope_gift_amount = red_envelope_gift_amount + ? WHERE `phone` = ? ",
      [redenvelope.money, redenvelope.money, userInfo.phone],
    );
    await connection.query(
      "INSERT INTO claimed_rewards (reward_id, phone, amount, type, time, status) VALUES (?, ?, ?, ?, ?, ?)",
      [generateClaimRewardID(), userInfo.phone, redenvelope.money, "Gift Code Bonus", time, 1]
    );
...
const callback_bank = async (req, res) => {
  ...
  if (status == 2) {
    ...
    await commissionController.updateDeposit(info[0].phone, info[0].money);
    return res.status(200).json({
      message: 0,
      status: true,
    });
    ...
    try {
      ...
      if (
        ...
        if (apiRecord.status === "success") {
          ...
          await commissionController.updateDeposit(apiRecord.customer_mobile, money);
          // let rechargeBonus;
