  ...
  try {
    ...
    if (check >= 0) {
      ...
      await connection.execute(
        "UPDATE `users` SET `money` = `money` - ?, Game5dTotalBetAmount = Game5dTotalBetAmount + ?, totalBetAmount = totalBetAmount + ? WHERE `token` = ? ",
        [total, total, total, auth],
      );
      ...
      await vipController.payoutRealTimeRebate(userInfo.phone, total);
      return res.status(200).json({
...
const handling5D = async (typeid) => {
  ...
  for (let i = 0; i < order.length; i++) {
    ...
    const sql = "UPDATE `users` SET `money` = `money` + ?, game5d_totalWinningAmount = game5d_totalWinningAmount + ?,totalWinningAmount =totalWinningAmount + ? WHERE `phone` = ? ";
    await connection.execute(sql, [nhan_duoc,nhan_duoc,nhan_duoc, phone]);
  }
};
