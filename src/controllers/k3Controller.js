  ...
  try {
    ...
    if (check >= 0) {
      ...
      await connection.execute(
        "UPDATE `users` SET `money` = `money` - ?,k3GameTotalBetAmount = k3GameTotalBetAmount + ?, totalBetAmount = totalBetAmount + ? WHERE `token` = ? ",
...
async function plusMoney(game) {
  ...
  for (let i = 0; i < order.length; i++) {
    ...
    if (orders.typeGame == "total") {
      ...
      const sql = "UPDATE `users` SET `money` = `money` + ? ,k3_totalWinningAmount = k3_totalWinningAmount + ?,totalWinningAmount =totalWinningAmount + ? WHERE `phone` = ? ";
