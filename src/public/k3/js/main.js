   ...
   $.ajax({
      ...
      success: function (response) {
         ...
         if (response.status) {
            ...
            console.log("K3 Bet Socket Emit (G1):", { change, gameJoin: 1, listJoin, money, xvalue, game: $("html").attr("data-dpr") });
...
function sendGame2() {
   ...
   $.ajax({
      ...
      success: function (response) {
         ...
         if (response.status) {
            $(".moneyU").text(response.money + "")
            console.log("K3 Bet Socket Emit (G2):", { change, gameJoin: 2, listJoin, money, xvalue, game: $("html").attr("data-dpr") });
...
function sendGame3() {
   ...
   $.ajax({
      ...
      success: function (response) {
         ...
         if (response.status) {
            $(".moneyU").text(response.money + "")
            console.log("K3 Bet Socket Emit (G3):", { change, gameJoin: 3, listJoin, money, xvalue, game: $("html").attr("data-dpr") });
...
function sendGame4() {
   ...
   $.ajax({
      ...
      success: function (response) {
         ...
         if (response.status) {
            $(".moneyU").text(response.money + "")
            console.log("K3 Bet Socket Emit (G4):", { change, gameJoin: 4, listJoin, money, xvalue, game: $("html").attr("data-dpr") });
...
const displayResultHandler = ({ status, amount, period, result }) => {
    ...
    const periodEl = document.getElementById("period_pop");
    const resultContainer = document.querySelector(".modal-popup__result");

    if (status === 'win') {
        modal.classList.add("win");
        modal.classList.remove("loss");
        document.querySelector(".modal-popup__title").innerHTML = "Congratulations";
        amountEl.innerHTML = "₹" + amount;
        amountEl.style.display = "block";
        amountEl.classList.add("success");
        amountEl.classList.remove("fail");
        ...
        document.querySelector(".modal-popup__title").innerHTML = "Sorry";
        amountEl.style.display = "none";
    }

    periodEl.innerHTML = "Period: " + period;

    let resultHtml = "";
    if (result) {
        let results = String(result).split("");
        results.forEach(num => {
            resultHtml += `<div class="popup_result_img img${num}"></div>`;
        });
    }
