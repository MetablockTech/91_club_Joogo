socket.on("data-server-k3", function (msg) {
   ...
   if (msg) {
      ...
      if (checkData == msg.game) {
         console.log("Game match found for:", msg.game);
         pageno = 0
         limit = 10
         page = 1
         ...
         console.log("Triggering checkWinLoss in 1s for period:", Result.period);
         setTimeout(() => {
            checkWinLoss(Result);
         }, 1000);
      }
...
function checkWinLoss(lastGame) {
    console.log("Checking win/loss for game:", lastGame);
    $.ajax({
        type: "POST",
        ...
        success: function (response) {
            ...
            let betListData = response.data.gameslist;
            let lastGameBets = betListData.filter(game => String(game.stage) === String(lastGame.period));
            console.log("Bets for last period:", lastGameBets);
            ...
            if (lastGameBets.length > 0) {
                ...
                let winGames = lastGameBets.filter(game => Number(game.status) === 1);
                let winGamesMoney = winGames.reduce((acc, game) => acc + Number(game.get), 0);
                let lostGames = lastGameBets.filter(game => Number(game.status) === 2);
                let lostGamesMoney = lostGames.reduce((acc, game) => acc + Number(game.price), 0);
