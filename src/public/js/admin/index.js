...
function showListOrder4(list_orders, x) {
  ...
  let result = list_orders.map((list_orders) => {
    return (htmls += `
                    <tr>
                        <td>${list_orders.period}</td>
                        <td class="${list_orders.amount % 2 == 0 ? "text-red" : "text-green"}">${list_orders.amount}</td>
                        <td>${list_orders.amount < 5 ? "Small" : "Big"}</td>
                        <td>
                            <span class="dot ${list_orders.amount % 2 == 0 ? "red" : "green"}"></span>
                            ${list_orders.amount == 0 || list_orders.amount == 5 ? '<span class="dot violet"></span>' : ""}
                        </td>
                    </tr>
                    `);
  });
