...
function initGameLogics({
  ...
  function alertBox(join, cssValueNumber, addText) {
    ...
    $('.Betting__Popup-body-money-btn[data-money="1"]').addClass("bgcolor");
    $(".Betting__Popup-body-money-main").attr("data-current-money", "1");

    // Reset Quantity / X to 1
    let activeXData = "1";
    $("#van-field-1-input").val(activeXData);
    $("div.Betting__Popup-body-x-btn").removeClass("bgcolor");
    $(`div.Betting__Popup-body-x-btn[data-x="${activeXData}"]`).addClass("bgcolor");
  ...
  $(`#van-field-1-input`).off("change.input keyup.input");
  $(`#van-field-1-input`).on("change.input keyup.input", function (e) {
    e.preventDefault();

    let currentX = parseInt($("#van-field-1-input").val());

    // Handle NaN or empty values
    // if (isNaN(currentX) || currentX < 1) {
    //   currentX = 1;
...
socket.on("data-server-trx-wingo", async function (msg) {
  try {
    ...
    // Only proceed if we have bets and ALL of them are processed (status != 0)
    const isAllProcessed = lastGameBets?.every(game => game.status !== 0);

    if (lastGameBets && lastGameBets.length > 0 && isAllProcessed) {
      const winGames = lastGameBets.filter((game) => game.get > 0);
      const winGamesMoney = winGames.reduce((acc, game) => acc + parseFloat(game.get), 0);
      
      const lostGames = lastGameBets.filter((game) => game.get === 0);
      const lostGamesMoney = lostGames.reduce((acc, game) => acc + parseFloat(game.money), 0);
