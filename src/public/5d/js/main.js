...
function reload_money() {
  fetch("/api/webapi/GetUserInfo")
    ...
    .then((data) => {
      ...
      $("#balance_amount").text(`₹ ${parseFloat(data.data.money_user).toFixed(2)} `);
      $(".Loading").fadeOut(0);
    });
}
reload_money();
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (response.status == true) {
        $("#balance_amount").text("₹ " + parseFloat(response.money).toFixed(2));
        showMeJoin();
      }
    },
  });
      ...
      return `
         <div data-v-b4b99df8="" class="MyGameRecordList__C-item" onclick="openGameBetDetails(${index})">
          ...
          ? `<div data-v-b4b99df8="" class="MyGameRecordList__C-item-r ${data.status == 1 ? "success" : ""}">
                        ...
                        <span data-v-b4b99df8="">${data.status == 1 ? `₹${data.get.toFixed(2)}` : `-₹${data.money.toFixed(2)}`}</span>
                     </div>`
          : ""
        }
         </div>
...
const displayResultHandler = ({ status, amount, period, result }) => {
  if (lastProcessedPeriod === period) {
    console.log("Period already processed, skipping popup:", period);
    return;
  }
  ...
  if (status === STATUS_MAP.WIN) {
    $("#popup_win_rupees_display").html(`₹${parseFloat(amount).toFixed(2)}`);
...
function handleMyEmerdList() {
  $.ajax({
    ...
    success: function (response) {
      ...
      // Filter bets for the current period
      $.ajax({
        type: "POST",
        url: "/api/webapi/5d/GetNoaverageEmerdList",
        data: {
        ...
        dataType: "json",
        success: function (gameResponse) {
          let list_orders = gameResponse.data.gameslist;
          let currentPeriod = list_orders[0].period;
          
          let lastGameBets = data.filter(game => String(game.stage) === String(currentPeriod));

          if (lastGameBets.length > 0) {
            // Only proceed if ALL bets for this period are processed (status != 0)
            const isAllProcessed = lastGameBets.every(game => Number(game.status) !== 0);
            if (!isAllProcessed) return;

            let winGames = lastGameBets.filter(game => Number(game.get) > 0);
            let winGamesMoney = winGames.reduce((acc, game) => acc + parseFloat(game.get), 0);
            
            let lostGames = lastGameBets.filter(game => Number(game.get) === 0);
            let lostGamesMoney = lostGames.reduce((acc, game) => acc + parseFloat(game.money), 0);
