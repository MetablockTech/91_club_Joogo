...
const CreateWingo = async (req, res) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    await connection.execute(sql, ["20250211100010000", arr[i], timeNow]);
    const sql_1 =
      "INSERT INTO wingo SET period = ?, game = ?, amount = 0, status = 0, time = ?";
    await connection.execute(sql_1, ["20250211100010001", arr[i], timeNow]);
  }
...
const Create5D = async (req, res) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    await connection.execute(sql, ["20250211100010000", "23521", arr[i], timeNow]);
    const sql_1 =
      "INSERT INTO 5d SET period = ?, result = ?, game = ?, status = 0, time = ?";
    await connection.execute(sql_1, ["20250211100010001", "0", arr[i], timeNow]);
  }
...
const CreateK3 = async (req, res) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    await connection.execute(sql, ["20250211100010000", "235", arr[i], timeNow]);
    const sql_1 =
...
const Admin = async (req, res) => {
  ...
  await connection.execute(
    "INSERT INTO `admin_ac` (`id`, `wingo1`, `wingo3`, `wingo5`, `wingo10`, `k5d`, `k5d3`, `k5d5`, `k5d10`, `win_rate`, `telegram`, `cskh`, `app`, `popup_image`, `popup_title`, `popup_text`, `popup_btn_text`) VALUES (NULL, '-1', '-1', '-1', '-1', '-1', '-1', '-1', '-1', '80', 'https://t.me/dreamsister', 'https://t.me/ChenQiaoYing', '#', '', 'Welcome Bonus', 'Get <strong>100%% Bonus up to ₹5000</strong> on your first deposit!', 'Claim Now')",
  );
};
